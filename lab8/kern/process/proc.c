...
load_icode_read(int fd, void *buf, size_t len, off_t offset)
{
    ...
    // 如果读取的数据长度不等于要求的长度len，则load_icode_read返回错误码-1；如果读取过程中发生了错误，则load_icode_read返回sysfile_read函数的错误码
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    //(3.1) read raw data content in file and resolve elfhdr
    // 根据应用程序文件的文件描述符（fd）通过调用load_icode_read()函数来加载和解析此ELF格式的执行程序
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum++)
    {
        ...
        if (vm_flags & VM_EXEC)
            perm |= PTE_X;
        //(3.3) call mm_map to build vma related to TEXT/DATA
        // 调用mm_map函数根据ELF格式的执行程序说明的各个段（代码段、数据段、BSS段等）的起始位置和大小建立对应的vma结构，并把vma插入到mm结构中，从而表明了用户进程的合法用户态虚拟地址空间
...
// 我们的终端需要实现这样的功能: 根据输入的程序名称, 从文件系统里加载对应的程序并执行
// 我们采取fork() exec()的方式来加载执行程序，exec()的一系列接口都需要重写来使用文件系统
// 以do_execve()为例，以前的函数原型从内存的某个位置加载程序：int do_execve(const char *name, size_t len, unsigned char *binary, size_t size) ;
// 现在则调用文件系统接口加载程序
